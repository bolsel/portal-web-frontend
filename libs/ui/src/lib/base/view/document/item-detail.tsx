...
export type UIBaseViewDocumentItemDetailType<Item extends Record<string, any>> =
  ...
  >;
export const UIBaseViewDocumentItemDetail = <Item extends Record<string, any>>(
  props: UIBaseViewDocumentItemDetailType<Item>['props']
) => {
  return UIBaseCreateCustomizable<UIBaseViewDocumentItemDetailType<Item>>({
    props,
    defaults: {
      fields: ({ item }) => ({
        title: item.title,
          ...
          {render('showDownloadLink') && (
            <div className="mt-5 flex items-center justify-center">
              <Link
                ...
                download
              >
                <UIBaseIcon icon="download" className="w-5 h-5" /> Unduh Dokumen
              </Link>
            </div>
          )}
        </div>
