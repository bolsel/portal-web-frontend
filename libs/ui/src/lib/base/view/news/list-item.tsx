...
export type UIBaseViewNewsListItemType<Item extends Record<string, any>> =
  IUIBaseCreateCustomizableDefine<
    {
      skeleton?: boolean;
      item: Item;
  ...
  >;
export const UIBaseViewNewsListItem = <Item extends Record<string, any>>(
  props: UIBaseViewNewsListItemType<Item>['props']
) => {
  return UIBaseCreateCustomizable<UIBaseViewNewsListItemType<Item>>({
    props,
    defaults: {
      small: () => false,
      fields: ({ item }) => ({
          ...
          >
            ...
            >
              <div className=" flex items-center justify-center w-full h-full transition-transform object-center object-cover duration-300 ease-in-out">
                <UIBaseIcon icon="image" className="text-gray-300 w-10 h-10" />
              </div>
            </div>
