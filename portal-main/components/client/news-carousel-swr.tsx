...
export const NewsCarouselSwr: FC<NewsCarouselProps & ApiResourceProps<'news'>> =
  ({pathQuery, paramsQuery, ...props}) => {
    ...
    return <UISwrComponent
      loadingComponent={()=><div className="bg-base-200 animate-pulse w-full h-full"></div>}
      noItemsComponent={()=><div className="bg-base-200 w-full h-full">Tidak ada data</div>}
      swrData={swrData}
    >
      {(data)=>{
