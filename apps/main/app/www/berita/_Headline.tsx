...
import { UIBaseIcon, UICarousel, UISwrResource } from '@portalweb/ui';
...
export default function Headline() {
  ...
  return (
    <UISwrResource
      resourceKey="news"
      loadingComponent={() => (
        <div className="h-[650px] bg-gray-500 relative animate-pulse"></div>
      )}
      pathQuery={
    ...
    >
      {({ data }) => {
        return (
          <UICarousel
            items={data!.data.map((d) => ({ ...d, image: d.image_cover.url }))}
            customizes={{
              hoverAnimate: () => true,
              autoPlay: () => true,
              pauseOnHover: () => false,
