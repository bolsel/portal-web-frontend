...
export default function Info({ data }) {
  const ListData = ({ title, value, icon, className }) => (
    ...
    >
      <UIBaseIcon icon={icon} className="w-5 h-5 my-[5px] mr-[5px]" />
    ...
    dataList.push({
      ...
      value: (
        <div className="flex flex-col p-2 w-full">
          {data.links.map((p, i) => {
            let _icon: UIBaseIconNamesType = 'web';
            ...
            return (
              <Link key={i} href={p.link} target="_blank">
                ...
                >
                  ...
                  <UIBaseIcon icon={_icon} className="w-6 h-6" />
                </button>
              </Link>
            );
          })}
        ...
        <div className="flex gap-2">
          {data.social_media.map((sm, i) => {
            ...
            return (
              ...
              >
                <UIBaseIcon icon={_icon} fallback="link" className="w-8 h-8" />
              </Link>
            );
          })}
        </div>
