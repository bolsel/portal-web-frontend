...
import { UICarousel, UIIcon, UISwrResource } from '@portalweb/ui';
import clsx from 'clsx';
import Link from 'next/link';
import { AnimatePresence, motion } from 'framer-motion';

export default function CarouselLatestNews() {
  return (
    <UISwrResource
      resourceKey="news"
      pathQuery={['latest']}
      paramsQuery={{ limit: 5 }}
      loadingComponent={() => (
        <div className="w-full rounded-lg h-[536px] animate-pulse bg-gray-300"></div>
      )}
    >
      {({ data }) => (
        <UICarousel
          ...
          customizes={{
            ...
            mainClass: ({ defaults, items }) =>
              clsx(
                defaults.mainClass,
                'bg-gray-200 w-full rounded-lg h-[536px]'
              ),
            ContentComponent() {
