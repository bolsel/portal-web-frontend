...
import LibDataNewsListDataView, {
  LibDataNewsListDataViewProps,
} from '../data/news/list/data-view';
...
export type LibSwrDataNewsListProps = Omit<
  LibDataNewsListDataViewProps,
  'items'
> &
  ApiResourceProps<'web_news'> & {
    noPagination?: boolean;
  };
export default function LibSwrDataWebNewsList({
  pathQuery,
  paramsQuery,
  noPagination,
  ...props
  ...
  const Skeleton = () => (
    <LibBaseDataView {...props}>
      {({ viewType }) => {
        const _items: any = [];
        const limit = paramsQuery?.limit ?? 4;
        for (let i = 0; i < limit; i++) {
          _items.push(
            <LibDataNewsListItemSkeleton
              key={i}
              viewType={viewType}
              {...props.itemComponent}
            />
  ...
  return (
    ...
    >
      {({ data, meta }) => {
        return (
          <>
            <LibDataNewsListDataView {...props} webNews items={data} />
            {meta && !noPagination ? (
              <div className={'mt-4'}>
                <UIPagination
