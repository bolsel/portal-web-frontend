...
export default function LibContentBlocks({data}: LibContentBlocksProps) {
  ...
  blocks.forEach((block, i) => {
    if (block.type === 'image') {
      ...
      items.push(<HeaderLevel level={block.data.level} key={i}>{block.data.text}</HeaderLevel>
      );
      ...
      const ListType = ({style, items, children = false}) => {
        const _items = items.map((item, index) => <li key={index} className='cdx-nested-list__item'>
          <div className='cdx-nested-list__item-body'>
            <div className='cdx-nested-list__item-content'>{item.content}</div>
            {item.items ? <ListType children style={style} items={item.items}/> : null}
          </div>
        </li>)
        if (style === 'unordered') {
      ...
      items.push(
        <ListType items={block.data.items ?? []} style={block.data.style} key={i}/>
      );
    }
  });
  return <div className='contents-blocks'>
