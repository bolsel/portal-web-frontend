...
type ComponentType = FC<{ item: Record<string, any>, viewType?: LibBaseDataViewViewType }>
...
export default function LibDataNewsListItem(
  ...
  >
    <Link
      ...
      href={isPortalBerita ? `https://bolselkab.go.id/berita/${item.slug}` : `/berita/${item.slug}`}
      rel="noopener"
    ...
    <div className={clsx(
      ...
      <Link
        ...
        title={item.title}
      >
        {titleComponent ? titleComponent({item, viewType}) : <h1
          className="font-lato font-medium text-lg leading-7 text-blue-gray-800 mb-[6px] group-hover:text-primary-700 line-clamp-2">
          {item.title}
        </h1>}
      </Link>
      {viewType === 'list' && small ? '' :
      ...
      <p className="font-normal text-xs leading-5 text-gray-500">
        <span className="capitalize flex items-center justify-center gap-2">
                  {!webNews ?
                    `${item.category.name} | ${item.publish_date_format}` :<><Icon icon="mdi:calendar" className="w-4 h-4"/> {item.publish_date_format}</>
                    // (item.tags ?? []).map((tag, i) => {
